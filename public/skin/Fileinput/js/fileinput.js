...
(function (factory) {
    ...
    previewCache = {
        ...
        out: function (id) {
		   ...
		   return {content: '<div class="file-live-thumbs">' + html + '</div>', caption: caption};
        },
        footer: function (id, i, isDisabled, size) {
            ...
            var config = data.config[i], caption = ifSet('caption', config), width = ifSet('width', config, 'auto'),
                url = ifSet('url', config, false), key = ifSet('key', config, null),show = ifSet('show', config, ''),
				param = ifSet('param', config, ''), param1 = ifSet('param1', config, ''), 
            ...
            return footer.replace(/\{caption}/g, caption).replace(/\{size}/g, data.getSize(size))
			    .replace(/\{param}/g, param).replace(/\{param1}/g, param1).replace(/\{show}/g, show)
    ...
    defaultFileActionSettings = {
        ...
        indicatorSuccess: '<i class="zmdi zmdi-check-circle zmdi-hc-fw text-success"></i>',
    ...
    tActions = '<div class="file-actions">\n' +
        ...
        '</div>';
    //noinspection HtmlUnknownAttribute
    tActionDelete = '<button type="button" class="zmdi zmdi-delete zmdi-hc-fw btn-icon-mini btn-round btn-sm bg-red  {removeClass}" ' + 'title="{removeTitle}">{removeIcon}</button>{dataUrl}\n';
    tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
        '{uploadIcon}</button>';
    /*tActionZoom = '<button type="button" class="zmdi zmdi-zoom-in zmdi-hc-fw  {zoomClass}" title="{zoomTitle}">{zoomIcon}</button>';*/
    ...
    FileInput.prototype = {
        ...
        _initSortable: function () {
			...
			$el = $preview.find('.file-live-thumbs');
        ...
        _initZoomButton: function () {
            var self = this,$first, $last,
                frames = self.$preview.find('.file-preview-frame').toArray(), len = frames.length,
                $prev = self.$preview.find('.zmdi-long-arrow-left'), $next = self.$preview.find('.zmdi-long-arrow-right'),
				$heart = self.$preview.find('.kv-file-heart');
            ...
            $first.find('.zmdi-long-arrow-left').addClass('hide');
		    $last.find('.zmdi-long-arrow-right').addClass('hide');
			
            self.$preview.find('.zmdi zmdi-zoom-in zmdi-hc-fw').each(function () {
                var $el = $(this);
			...
			$heart.each(function () {
                ...
                handler($el, 'click', function () {
					var $frame = $el.closest('.file-preview-frame');
					var show = $frame.find('input[name="show[]"]').val();
					//alert(show);
					if(show==''){
					  $el.find('i').removeClass('glyphicon-heart').addClass('glyphicon-heart-empty');
					  $frame.find('input[name="show[]"]').val('-empty');
					}
					else{
					  $el.find('i').removeClass('glyphicon-heart-empty').addClass('glyphicon-heart');
					  $frame.find('input[name="show[]"]').val('');
						}
                });
            });
        ...
        _initPreviewActions: function () {
            var self = this, deleteExtraData = self.deleteExtraData || {},
                resetProgress = function () {
                    ...
                    if (self.$preview.find('.zmdi-delete').length === 0 && !hasFiles) {
                        self.reset();
                        self.initialCaption = '';
                    }
                };
            self._initZoomButton();
            self.$preview.find('.zmdi-delete').each(function () {
                var $el = $(this), vUrl = self.deleteUrl, vKey = $el.data('key');
               ...
               // }
                ...
                var val = $frame.find('input[type="hidden"]').val();
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs('.file-preview-success').each(function () {
                var $thumb = $(this), $remove = $thumb.find('.zmdi-delete');
				var vUrl = self.deleteUrl;
                ...
                handler($remove, 'click', function () {
					...
					var val = $thumb.find('input[type="hidden"]').val();
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            if (self.showPreview) {
                ...
                $btnDelete = $thumb.find('.zmdi-delete');
        ...
        _uploadBatch: function () {
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                if (isEmpty(data) || isEmpty(data.error)) {
                    ...
                    if (self.showPreview) {
                        $thumbs.each(function () {
                            var $thumb = $(this), $btnDelete = $thumb.find('.zmdi-delete'),
                                $btnUpload = $thumb.find('.kv-file-upload');
                            $thumb.removeClass('file-uploading');
                            $btnUpload.removeAttr('disabled');
                            $btnDelete.removeAttr('disabled');
                ...
                if ($preview.length && FileReader !== undefined) {
                    ...
                    reader.onloadend = function () {
                        msg = msgProgress.replace('{index}', i + 1).replace('{files}', numFiles)
                            .replace('{percent}', 506).replace('{name}', caption);
                        setTimeout(function () {
                            $status.html(msg);
							readFile(i + 1);
                            self._updateFileDetails(numFiles);
        ...
        _renderFileFooter: function (caption, size, width, isError) {
            ...
            if (self.isUploadable) {
                out = template.replace(/\{actions}/g, self._renderFileActions(upl, rem, zoom, drg, false, false, false))
					...
					.replace(/\{param}/g, '').replace(/\{param1}/g, '').replace(/\{show}/g, '')
                ...
                if (!isAjaxUpload || flagSingle) {
                    ...
                    if (isAjaxUpload && ctr === 0 && (!previewCache.count(self.id) || self.overwriteInitial)) {
                        if(self.autoReplace){
                            /*进行更新处理*/
                            var vUrl = self.deleteUrl;
                            self.$container.parent().find('input[type="hidden"]').each(function(){
                            var val = $(this).val();
                            $.ajax({
                                      type: "post",
                                      url: vUrl,
        ...
        cancel: function () {
            ...
            self._getThumbs().each(function () {
                ...
                if (self.filestack[ind] !== undefined) {
                    ...
                    $thumb.find('.zmdi-delete').removeClass('disabled').removeAttr('disabled');
                }
                self.unlock();
            });
            return self.$element;
